...
send_file(
    ...
    for (packet_num=0; packet_num < num_packets; ) {
        ...
        if (response != RESPONSE_ACK) {
            ...
            printf("received response %s from sending packet %d\n",
                   response_to_string(response), packet_num);
        }
        ...
        switch (state) {
            case STATE0:
                state = (c == 0xf0)
                    ? STATE1
                    : STATE0;
                printf("read first byte of response\n");
                break;
            ...
            case STATE1:
                state = (c == 0x7e)
                    ? STATE2
                    : STATE0;
                printf("read second byte of response\n");
                break;
            ...
            case STATE2:
                state = (c == channel_num)
                    ? STATE3
                    : STATE0;
                printf("read channel num from response (%X)\n",
                       channel_num);
                break;
            ...
            case STATE3:
                state = (c >= 0x7c && c <= 0x7f)
                    ...
                    : STATE0;
                x = c;
                printf("read fourth byte of response (%X)\n", x);
                break;
            ...
            case STATE4:
                state = (c == modded_packet_num)
                    ? STATE5
                    : STATE0;
                printf("read modded_packet_num from response (%X)\n", c);
                break;

            case STATE5:
                if (c == 0xf7) {
                    printf("read last byte of response\n");
                    done = 1;
